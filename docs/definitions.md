...
| Daylight-saving window | On a date when Europe/Stockholm changes its UTC offset, the local clock hours 02:00–03:59. A stop event whose timetable clock time falls in this window, written 02:00:00–03:59:59 on that date's service or 26:00:00–27:59:59 on the previous date's service, gets status `dst_ambiguous`. It is excluded from punctuality and coverage, and counted separately (D-021) | FIXED |
| Schedule mismatches | Stop events, on trips assigned to the service date and outside the daylight-saving window, where the feed's scheduled time (time minus delay) differs from the pipeline's scheduled time in at least one stop time update. Counted per service date in data_quality. Expected to be zero (D-021) | FIXED |
...
| Unobserved stop events | Scheduled stop events that are not on a cancelled trip, not skipped (D-009), not in the daylight-saving window (D-021), and have no realtime observation, including those whose last realtime value lacks the recorded-time marker (D-007). They are labelled `unobserved`, **never** counted as on time and never silently dropped | FIXED |
| Stop event status | Every scheduled stop event at the measurement point gets exactly one status, checked in this order: `out_of_scope` (on an out-of-scope trip, D-013), `cancelled` (on a cancelled trip), `skipped`, `dst_ambiguous` (in the daylight-saving window, D-021), `observed`, `unobserved` (D-009) | FIXED |
| Coverage | Observed stop events ÷ scheduled stop events at the measurement point (final stops excluded, D-008), per route and day. Stop events on cancelled trips, skipped stop events (D-009) and stop events in the daylight-saving window (D-021) are excluded from both counts, since each is reported separately. Reported alongside every punctuality figure | FIXED |
...
| Added trips | Realtime trips with start_date D that match no scheduled trip on D (D-011). They are counted per service date in data_quality as unmatched realtime trips and excluded from punctuality. The feed cannot tell a genuinely added trip from one tagged with the wrong service date; the latter was seen on 2025-10-25 and 2025-10-26. Expected to be zero; a non-zero count marks that date's pipeline run as a warning (D-021) | FIXED |
