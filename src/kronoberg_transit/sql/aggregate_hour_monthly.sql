-- hour_monthly (D-017): one row per (month, day_type, hour_local). Measure
-- block M only, over in-scope non-final stop_events grouped by the local
-- hour of the stop event's scheduled departure. Requires aggregate_base.sql
-- and aggregate_network_monthly.sql (for month_completeness) to have run
-- first.
...
se_agg AS (
    SELECT month, day_type, hour_local,
    ...
    FROM se_hour
    GROUP BY month, day_type, hour_local
)
SELECT
    sa.month, sa.day_type, sa.hour_local, mc.month_complete,
